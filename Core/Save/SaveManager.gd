...
func create_new_game_save(slot_name: String = DEFAULT_SLOT_NAME, start_scene_path: String = "res://Content/World/BaseWorld.tscn") -> Dictionary:
	ensure_save_directory()
	var save_data: Dictionary = new_empty_save()
	...
	if normalized_scene_path.is_empty():
		normalized_scene_path = "res://Content/World/BaseWorld.tscn"
...
func reapply_player_state_deferred(scene_root: Node, frames: int = 2) -> void:
	if not is_instance_valid(scene_root):
		return
	call_deferred("_reapply_player_state_after_frames", scene_root, frames)
